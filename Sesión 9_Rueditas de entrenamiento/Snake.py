...
# Colores. Cada color se define como una tupla de valores RGB       https://htmlcolorcodes.com/es/
WHITE = (255,255,255)
BLACK = (0,0,0)
RED = (255,0,0)
GREEN = (0,255,0)
...
food_spawn = True       # Indica si la comida está disponible o debe generarse nuevamente.

# Puntuación, o el contador de puntos
score = 0

# Fuente para mostrar la puntuación en pantalla
font = pygame.font.SysFont('consolas',20)

# Función para mostrar la puntuación en pantalla
def show_score():
    score_surface = font.render(f'Score: {score}', True, WHITE)     # font.render Crea texto con la puntuación
    score_rect = score_surface.get_rect()       # Capturar la puntuación
    score_rect.midtop = (WIDTH // 2, 10)        # Ubicar en el espacio de la pantalla
    screen.blit(score_surface,score_rect)       # blit  Dibuja el texto en la pantalla

# Bucle principal
while True:
    # Manejar eventos
    for event in pygame.event.get():    # Capturar eventos, como cerrar la ventana o pulsar teclas
        if event.type == pygame.QUIT:  # Sale del programa 
            pygame.quit()
            sys.exit()
        if event.type == pygame.KEYDOWN:        # Detectar si se presiona una tecla
    ...
    snake_body.insert(0, list(snake_pos))       # Anadir un nuevo bloque a la cabeza
    ...
    # Mostrar la puntuación, llamar a la función para mostrar la puntuación
    show_score()

    # Terminar el juego si la snake choca, detecta colisiones
    ...
    clock.tick(25)              # Establece la velocidad del juego 
